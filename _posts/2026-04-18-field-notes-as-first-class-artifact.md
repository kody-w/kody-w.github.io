...
I keep a `docs/field-notes/` directory in the platform repo. Every workstream gets its own dated HTML page. Not Markdown. HTML, self-contained, with a dark theme, dropped at `YYYY-MM-DD-{slug}.html` with an index page that lists them.
...
Per-workstream HTML pages, indexed in one place, written before push. That's the whole pattern. Ship it on day one of any nontrivial repo.
