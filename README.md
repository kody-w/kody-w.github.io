...
- **`ai-brainstem.html`** — Self-contained, prompt-first AI Brainstem training quest with 24 checkpoints, local-only progress, an agent.py-only capability story, the pinned prompt-only Gauntlet proof, categorized SDK state, explicit gates, and final pinned implementation sources.
